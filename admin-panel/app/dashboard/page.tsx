...
export default function DashboardPage() {
  ...
  useEffect(() => {
    ...
    const fetchDashboardData = async () => {
      try {
        ...
        const activities = Array.isArray(dashboardData?.recent_activities)
        ...
        if (backendPolicy === 'individual_ads' || backendPolicy === 'captain_ads') {
          setRegistrationPolicy(backendPolicy);
        } else {
          setRegistrationPolicy(DEFAULT_REGISTRATION_POLICY);
        }
        ...
        setRecentActivities([]);
        setRegistrationPolicy(DEFAULT_REGISTRATION_POLICY);
        setPolicyError('Unable to load the current registration policy.');
      } finally {
        if (mounted) {
          setLoading(false);
  ...
  const handleRegistrationPolicyChange = async (
    ...
    try {
      await adminAPI.updateSetting('registration_policy', {
        key: 'registration_policy',
        value: newPolicy,
        description: 'Registration ad policy used for new tournament registrations.',
        value_type: 'string',
      });

      setPolicyMessage('Registration policy updated successfully.');
    } catch (err) {
      console.error('Error updating registration policy:', err);
      setRegistrationPolicy(previousPolicy);
      setPolicyError('Failed to update registration policy. Please try again.');
    } finally {
        ...
        <div className="grid grid-cols-1 gap-6 lg:grid-cols-2">
          <div className="rounded-[28px] border border-white/10 bg-white/5 p-6 shadow-2xl backdrop-blur-xl">
            ...
            <div className="mt-4 space-y-3">
              <Link href="/tournaments/new" className="block rounded-2xl bg-gradient-to-r from-violet-500 to-cyan-400 px-4 py-3 font-semibold text-white transition hover:opacity-95">Create Tournament</Link>
              <Link href="/users" className="block rounded-2xl border border-white/10 bg-white/5 px-4 py-3 font-semibold text-white transition hover:bg-white/10">Manage Users</Link>
              <Link href="/registrations" className="block rounded-2xl border border-white/10 bg-white/5 px-4 py-3 font-semibold text-white transition hover:bg-white/10">View Registrations</Link>
            </div>
          </div>
          ...
          <div className="rounded-[28px] border border-white/10 bg-white/5 p-6 shadow-2xl backdrop-blur-xl">
            <div className="flex items-start justify-between gap-4">
              <div>
                ...
                <p className="mt-3 text-sm text-white/70">Choose whether tournaments use individual player ads or captain ads for registration.</p>
              </div>
              {policySaving ? <span className="rounded-full border border-cyan-400/20 bg-cyan-400/10 px-3 py-1 text-xs font-medium text-cyan-200">Saving...</span> : null}
            </div>
            ...
            {policyLoading ? (
              <p className="mt-3 text-xs text-white/50">Loading current policy...</p>
            ) : policySaving ? (
              <p className="mt-3 text-xs text-cyan-200/70">Saving policy to the server...</p>
            ) : policyMessage ? (
              <p className="mt-3 text-xs text-emerald-300">{policyMessage}</p>
            ) : policyError ? (
              <p className="mt-3 text-xs text-red-300">{policyError}</p>
            ) : (
        ...
        <div className="mt-8 rounded-[28px] border border-white/10 bg-white/5 p-6 shadow-2xl backdrop-blur-xl">
          <div className="flex items-center justify-between gap-4">
            <div>
              ...
              <p className="mt-1 text-sm text-white/50">Latest activity from the platform</p>
            </div>
            {loading ? <span className="text-xs text-white/50">Loading...</span> : null}
          </div>

          <div className="mt-4 space-y-3">
            {!loading && recentActivities.length === 0 ? (
              <div className="rounded-2xl border border-white/10 bg-white/5 px-4 py-8 text-center">
                <p className="text-sm text-white/60">No recent activities</p>
              </div>
            ) : null}

            {recentActivities.map((activity) => (
              <div key={activity.id} className="flex items-center justify-between gap-4 rounded-2xl border border-white/10 bg-white/5 px-4 py-3">
                <div className="min-w-0">
                  <p className="font-medium text-white">{activity.title}</p>
                  <p className="truncate text-sm text-white/55">{activity.description}</p>
                </div>
                <span className="shrink-0 text-xs text-white/50">{formatRelativeTime(activity.created_at)}</span>
              </div>
            ))}
          </div>
        </div>
