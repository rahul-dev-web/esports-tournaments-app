...
class _Details extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    final progress = tournament.totalSlots == 0
        ? 0.0
        : (tournament.registeredTeams / tournament.totalSlots).clamp(0.0, 1.0);
      ...
      children: [
        Container(
          ...
          decoration: BoxDecoration(
            gradient: const LinearGradient(
              colors: [Color(0xFF1B2140), Color(0xFF10182F)],
            ),
          ...
          child: Column(
            ...
            children: [
              ...
              Text(
                '${tournament.mode} • ${tournament.typeLabel}',
        ...
        _Section(
          ...
          children: [
            _InfoRow(
              icon: Icons.schedule,
              label: 'Starts',
              value: _formatDate(tournament.startsAt),
            ),
            _InfoRow(
              icon: Icons.groups_outlined,
              label: 'Team size',
              value: '${tournament.teamSize} players',
            ),
            _InfoRow(
              icon: Icons.emoji_events_outlined,
              label: 'Reward',
              value: tournament.reward.isEmpty ? 'TBA' : tournament.reward,
            ),
            _InfoRow(
              icon: Icons.confirmation_number_outlined,
              label: 'Entry',
              value: tournament.entryRequirement.isEmpty
                  ? 'No requirement'
        ...
        _Section(
          ...
          children: [
            ...
            const SizedBox(height: 6),
            Text(
              tournament.adsRequired > 0
                  ? '${tournament.adsRequired} rewarded ad${tournament.adsRequired == 1 ? '' : 's'} required.'
        ...
        FilledButton.icon(
          onPressed: isOpen
              ? () => Navigator.of(context).push(
                    MaterialPageRoute(
                      builder: (_) =>
                          RegisterTournamentPage(tournament: tournament),
                    ),
                  )
              : null,
          icon: const Icon(Icons.how_to_reg),
          label: Text(
            isOpen ? 'Register with my team' : tournament.status.name.toUpperCase(),
          ),
        ),
        if (!isOpen)
          const Padding(
...
class _InfoRow extends StatelessWidget {
  ...
  Widget build(BuildContext context) => Padding(
        ...
        child: Row(
          children: [
            Icon(icon, color: const Color(0xFF8A5CFF), size: 20),
